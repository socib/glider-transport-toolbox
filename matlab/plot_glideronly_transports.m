...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                                         %  
% function plot_glideronly_transports                                                     %
%                        (missionList,config,datamode,sectionName,logo,inputDir,imageDir) %
%                                                                                         %
% Purpose:                                                                                %
...
%                                                                                         %
% Outputs:                                                                                %
% - figures of geostrophic transports                                                     %
%                                                                                         %
% Author: Melanie Juza - SOCIB                                                            %
%         mjuza@socib.es                                                                  %
%                                                                                         %
% Date of creation: Feb-2018 (last modification: 28-Feb-2018)                             %     
%                                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             'canalesFeb2019';'canalesApr2019';'canalesMay2019';'canalesJul2019';'canalesSep2019'};
%datamode='dt';
%sectionName='MallorcaChannel';     
%logo='socib';       
%inputDir='/LOCALDATA/GLIDERS/Transports/Transports_smooth24_critWIW_fixed-range_onlyglider';
...
if option.glidertotal
  ...
  disp('Plotting net/north/south of total flow')

  % Total flow 
  figure('visible','on'); initfigall(45,30); clf;
  for itrans=1:3
  plot(dates,squeeze(transport(itrans,1,nbwm+1,:)),'color',col2{itrans},'linestyle','-','marker','.','markersize',20,'linewidth',1); hold on; 
  end
  legend(['Northward flow (mean/std=',num2str(meanT(1,1),'%2.2f'),'/',num2str(stdT(1,1),'%2.2f'),')'],...
         ['Southward flow (mean/std=',num2str(meanT(2,1),'%2.2f'),'/',num2str(stdT(2,1),'%2.2f'),')'],...
         ['Net flow (mean/std=',      num2str(meanT(3,1),'%2.2f'),'/',num2str(stdT(3,1),'%2.2f'),')'],'fontsize',font,'Location','NorthWest')
  ...
  datetick('x','mmmyy')
  axis([dates2(1) dates2(end) mint maxt]);
  ...
  % Print
  figurepng=strcat([imageDir,'Transports_',sectionName,'_',datestr(dates2(1),'yyyymm'),'-',datestr(dates2(end),'yyyymm'),'_',dataset,'_',strwater{iw},extfig])
...
if option.watermasses
  ...
  disp('Plotting Glider north/south of watermass flows')
  % Water mass flows
  figure('visible','on'); initfigall(45,30); clf;
  for iw=1:nbwm+1
    sizemark=1; if iw==nbwm+1 sizemark=17;, end
    if iw==2 || iw==4
    width=2;
    else
    width=1;
    end
    plot(dates,squeeze(transport(1,1,iw,:)),'color',col{iw},'linestyle','-','marker','.','markersize',sizemark,'linewidth',width); hold on; 
  end
  legend([strstat_wm{1}],[strstat_wm{2}],[strstat_wm{3}],[strstat_wm{4}],[strstat_wm{5}],[strwater{6}],'fontsize',14,'Location','NorthWest')
  for iw=1:nbwm+1
    sizemark=1; if iw==nbwm+1 sizemark=17;, end
    if iw==2 || iw==4
    width=2;
    ...
    plot(dates,squeeze(transport(2,1,iw,:)),'color',col{iw},'linestyle','-','marker','.','markersize',sizemark,'linewidth',width); hold on; 
  end
  axis([dates(1) dates(end) mint maxt]);grid on;
  ylabel('Transport / section (Sv)','fontsize',font)
  title([dataset,' transport in the ',sectionName,' from ',datestr(dates2(1),'mmm-yyyy'),' to ',datestr(dates2(end),'mmm-yyyy')],'fontsize',font)
  set(gca,'fontsize',font)
  datetick('x','mmmyy')
  ...
  set(gcf,'renderer','zbuffer');
  % Print
  figurepng=strcat([imageDir,'Transports_',sectionName,'_',datestr(dates2(1),'yyyymm'),'-',datestr(dates2(end),'yyyymm'),'_',dataset,'_watermass',extfig])
  print('-dpng','-r300','-painters',figurepng)
